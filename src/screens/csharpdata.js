...
export default function Csharpdata() {
    ...
    let usewebdata = webdata && webdata.filter(m=>m.id ==="csharpdata-videos");
    ...
    useEffect(() => {
      ...
      settitle(usewebdata&&usewebdata[0].videos[0].title)
        ...
        //quiz code
        let usequizzes = webdata && webdata.filter(m=>m.id ==="csharpdata-quizzes");
        ...
        const quizclickhandle=async(tag)=>{
            ...
            function msToTime(s) {
              ...
              if(data && data[0].csharpdataq ==undefined){
                setcurrenttag(tag);
            ...
            else if(data[0].csharpdataq[tag] ===undefined)
            {
            ...
            else if((data && data[0].csharpdataq ==undefined) || (data && data[0].csharpdataq[tag]< new Date().getTime()))
            {
                setcurrenttag(tag);
            setright(usequizzes && usequizzes[0][currenttag][currentq].correcta);
            setquizcheck(quizcheck+1);
            ...
            else
            {
                if(localStorage.getItem("codetime")>new Date().getTime())
                {
                    Swal.fire({
                        icon: 'error',
                        title: 'حاول مجددا بعد ' +msToTime( data && data[0].csharpdataq[tag]- new Date().getTime())
                    })
                }
                else
                {
        ...
        const TheEndOfQuiz=async()=>{
            if(score/lengthq>=0.5)
            {
                //xp
                let xp= score*100;
                if(data[0].csharpdataxp ==undefined || data[0].csharpdataxp[currenttag]<xp || data[0].csharpdataxp[currenttag] === undefined)
                {
                    let newFields;
                    ...
                    {
                        if(data[0].csharpdataxp !== undefined){
                            if(data[0].csharpdataxp[currenttag] !== undefined){
                                newFields={exp:(data[0].exp+xp-data[0].csharpdataxp[currenttag])}
                            }
                            else{
                    ...
                    let new2Fields;
                    if(data[0].csharpdata!==undefined)
                    {
                        new2Fields = {csharpdata:{
                            ...data[0].csharpdata,
                            [currenttag]:true,
                            [nexttag]:false
                        }};
                    ...
                    <div className="listside">
    ...
    
                        {
                            usewebdata && usewebdata[0].videos.map((item,i) => {
                                ...
                                else if(i===0 &&data[0].csharpdata==undefined)
                                {
                                    return(
                                        <Videol status="f" title={item.title} keyt={i} key={i} handleclick={clickhandle} quizclick={quizclickhandle}/>
                                        )
                                }
                                else if(data[0].csharpdata && data[0].csharpdata[i]===false){
                                    return(
                                    <Videol status="f" title={item.title} keyt={i} key={i} handleclick={clickhandle} quizclick={quizclickhandle}/>
                                    )
                                }else if(data[0].csharpdata && data[0].csharpdata[i] === true)
                                {
                                    return <Videol status="t" title={item.title} keyt={i} key={i} handleclick={clickhandle} quizclick={quizclickhandle}/>
                                }else if(data[0].csharpdata ==undefined || (data[0].csharpdata && data[0].csharpdata[i] === undefined))
                                {
