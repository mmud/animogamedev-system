...
export default function Cplusplus() {
    ...
    let usewebdata = webdata && webdata.filter(m=>m.id ==="cplusplus-videos");
    ...
    useEffect(() => {
      ...
      settitle(usewebdata&&usewebdata[0].videos[0].title)
        ...
        //quiz code
        let usequizzes = webdata && webdata.filter(m=>m.id ==="cplusplus-quizzes");
        ...
        const quizclickhandle=async(tag)=>{
            ...
            function msToTime(s) {
              ...
              if(data && data[0].cplusplusq ==undefined){
                setcurrenttag(tag);
            ...
            else if(data[0].cplusplusq[tag] ===undefined)
            {
            ...
            else if((data && data[0].cplusplusq ==undefined) || (data && data[0].cplusplusq[tag]< new Date().getTime()))
            {
                setcurrenttag(tag);
            setright(usequizzes && usequizzes[0][currenttag][currentq].correcta);
            setquizcheck(quizcheck+1);
            ...
            else
            {
                if(localStorage.getItem("codetime")>new Date().getTime())
                {
                    Swal.fire({
                        icon: 'error',
                        title: 'حاول مجددا بعد ' +msToTime( data && data[0].cplusplusq[tag]- new Date().getTime())
                    })
                }
                else
                {
        ...
        const TheEndOfQuiz=async()=>{
            if(score/lengthq>=0.5)
            {
                //xp
                let xp= score*100;
                if(data[0].cplusplusxp ==undefined || data[0].cplusplusxp[currenttag]<xp || data[0].cplusplusxp[currenttag] === undefined)
                {
                    let newFields;
                    ...
                    {
                        if(data[0].cplusplusxp !== undefined){
                            if(data[0].cplusplusxp[currenttag] !== undefined){
                                newFields={exp:(data[0].exp+xp-data[0].cplusplusxp[currenttag])}
                            }
                            else{
                    ...
                    let new2Fields;
                    if(data[0].cplusplus!==undefined)
                    {
                        new2Fields = {cplusplus:{
                            ...data[0].cplusplus,
                            [currenttag]:true,
                            [nexttag]:false
                        }};
                    ...
                    <div className="listside">
    ...
    
                        {
                            usewebdata && usewebdata[0].videos.map((item,i) => {
                                ...
                                else if(i===0 &&data[0].cplusplus==undefined)
                                {
                                    return(
                                        <Videol status="f" title={item.title} keyt={i} key={i} handleclick={clickhandle} quizclick={quizclickhandle}/>
                                        )
                                }
                                else if(data[0].cplusplus && data[0].cplusplus[i]===false){
                                    return(
                                    <Videol status="f" title={item.title} keyt={i} key={i} handleclick={clickhandle} quizclick={quizclickhandle}/>
                                    )
                                }else if(data[0].cplusplus && data[0].cplusplus[i] === true)
                                {
                                    return <Videol status="t" title={item.title} keyt={i} key={i} handleclick={clickhandle} quizclick={quizclickhandle}/>
                                }else if(data[0].cplusplus ==undefined || (data[0].cplusplus && data[0].cplusplus[i] === undefined))
                                {
