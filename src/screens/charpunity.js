...
export default function Csharpunity() {
    ...
    let usewebdata = webdata && webdata.filter(m=>m.id ==="csharpunity-videos");
    ...
    useEffect(() => {
      ...
      settitle(usewebdata&&usewebdata[0].videos[0].title)
        ...
        //quiz code
        let usequizzes = webdata && webdata.filter(m=>m.id ==="csharpunity-quizzes");
        ...
        const quizclickhandle=async(tag)=>{
            ...
            if(data && data[0].csharpunityq ==undefined){
                setcurrenttag(tag);
            ...
            else if(data[0].csharpunityq[tag] ===undefined)
            {
            ...
            else if((data && data[0].csharpunityq ==undefined) || (data && data[0].csharpunityq[tag]< new Date().getTime()))
            {
                setcurrenttag(tag);
            setright(usequizzes && usequizzes[0][currenttag][currentq].correcta);
            setquizcheck(quizcheck+1);
            ...
            else
            {
                if(localStorage.getItem("codetime")>new Date().getTime())
                {
                    Swal.fire({
                        icon: 'error',
                        title: 'حاول مجددا بعد ' +msToTime( data && data[0].csharpunityq[tag]- new Date().getTime())
                    })
                }
                else
                {
        ...
        const TheEndOfQuiz=()=>{
            if(score/lengthq>=0.5)
            {
                //xp
                let xp= score*100;
                if(data[0].csharpunityxp ==undefined || data[0].csharpunityxp[currenttag]<xp || data[0].csharpunityxp[currenttag] === undefined)
                {
                    let newFields;
                    ...
                    {
                        if(data[0].csharpunityxp !== undefined){
                            if(data[0].csharpunityxp[currenttag] !== undefined){
                                newFields={exp:(data[0].exp+xp-data[0].csharpunityxp[currenttag])}
                            }
                            else{
                    ...
                    let new2Fields;
                    if(data[0].csharpunity!==undefined)
                    {
                        new2Fields = {csharpunity:{
                            ...data[0].csharpunity,
                            [currenttag]:true,
                            [nexttag]:false
                        }};
                    ...
                    const new4Fields = {csharpunityxp:{
                        ...data[0].csharpunityxp,
                        [currenttag]:xp,
                    }};
                    ...
                    <div className="listside">
    ...
    
                        {
                            usewebdata && usewebdata[0].videos.map((item,i) => {
                                ...
                                else if(i===0 &&data[0].csharpunity==undefined)
                                {
                                    return(
                                        <Videol status="f" title={item.title} keyt={i} key={i} handleclick={clickhandle} quizclick={quizclickhandle}/>
                                        )
                                }
                                else if(data[0].csharpunity && data[0].csharpunity[i]===false){
                                    return(
                                    <Videol status="f" title={item.title} keyt={i} key={i} handleclick={clickhandle} quizclick={quizclickhandle}/>
                                    )
                                }else if(data[0].csharpunity && data[0].csharpunity[i] === true)
                                {
                                    return <Videol status="t" title={item.title} keyt={i} key={i} handleclick={clickhandle} quizclick={quizclickhandle}/>
                                }else if(data[0].csharpunity ==undefined || data[0].csharpunity && data[0].csharpunity[i] === undefined)
                                {
