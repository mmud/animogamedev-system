...
import { query, where,updateDoc,doc,getDoc } from "firebase/firestore";  
...
export default function Cplusplusdata() {
    ...
    let usewebdata = webdata && webdata.filter(m=>m.id ==="cplusplusdata-videos");
    ...
    useEffect(() => {
      ...
      settitle(usewebdata&&usewebdata[0].videos[0].title)
        ...
        //quiz code
        let usequizzes = webdata && webdata.filter(m=>m.id ==="cplusplusdata-quizzes");
        ...
        const [lengthq, setlengthq] = useState(usequizzes && usequizzes[0][currenttag].length);
        ...
        const quizclickhandle=async(tag)=>{
            ...
            function msToTime(s) {
              ...
              if(data && data[0].cplusplusdataq ==undefined){
                setcurrenttag(tag);
            ...
            else if(data[0].cplusplusdataq[tag] ===undefined)
            {
            ...
            else if((data && data[0].cplusplusdataq ==undefined) || (data && data[0].cplusplusdataq[tag]< new Date().getTime()))
            {
                setcurrenttag(tag);
            setright(usequizzes && usequizzes[0][currenttag][currentq].correcta);
            setquizcheck(quizcheck+1);
            ...
            else
            {
                if(localStorage.getItem("codetime")>new Date().getTime())
                {
                    Swal.fire({
                        icon: 'error',
                        title: 'حاول مجددا بعد ' +msToTime( data && data[0].cplusplusdataq[tag]- new Date().getTime())
                    })
                }
                else
                {
        ...
        const TheEndOfQuiz=async()=>{
            if(score/lengthq>=0.5)
            {
                //xp
                let xp= score*100;
                if(data[0].cplusplusdataxp ==undefined || data[0].cplusplusdataxp[currenttag]<xp || data[0].cplusplusdataxp[currenttag] === undefined)
                {
                    let newFields;
                    ...
                    {
                        if(data[0].cplusplusdataxp !== undefined){
                            if(data[0].cplusplusdataxp[currenttag] !== undefined){
                                newFields={exp:(data[0].exp+xp-data[0].cplusplusdataxp[currenttag])}
                            }
                            else{
                    ...
                    let new2Fields;
                    if(data[0].cplusplusdata!==undefined)
                    {
                        new2Fields = {cplusplusdata:{
                            ...data[0].cplusplusdata,
                            [currenttag]:true,
                            [nexttag]:false
                        }};
                    }
                    ...
                    const new4Fields = {cplusplusdataxp:{
                        ...data[0].cplusplusdataxp,
                        [currenttag]:xp,
                    }};
            ...
            if(localStorage.getItem("codetime")>new Date().getTime())
            {
                ...
                let date = new Date();
                const new3Fields = {cplusplusdataq:{
                    ...data[0].cplusplusdataq,
        ...
        const [onecec, setonecec] = useState(false)
        if(webdata!=undefined && loadb!=null && loads!=null&&onecec==false){
            setonecec(true);
            setTimeout(() => {
                ...
                <div className="Quiz" >
                    <div className="quiz-app"ref={quizblock}>
                        ...
                        <div className="quiz-info">
                            <div className="category">Category: <span>C sharp</span></div>
                    ...
                    <div className="listside">
    ...
    
                        {
                            usewebdata && usewebdata[0].videos.map((item,i) => {
                                ...
                                else if(i===0 &&data[0].cplusplusdata==undefined)
                                {
                                    return(
                                        <Videol status="f" title={item.title} keyt={i} key={i} handleclick={clickhandle} quizclick={quizclickhandle}/>
                                        )
                                }
                                else if(data[0].cplusplusdata && data[0].cplusplusdata[i]===false){
                                    return(
                                    <Videol status="f" title={item.title} keyt={i} key={i} handleclick={clickhandle} quizclick={quizclickhandle}/>
                                    )
                                }else if(data[0].cplusplusdata && data[0].cplusplusdata[i] === true)
                                {
                                    return <Videol status="t" title={item.title} keyt={i} key={i} handleclick={clickhandle} quizclick={quizclickhandle}/>
                                }else if(data[0].cplusplusdata ==undefined || (data[0].cplusplusdata && data[0].cplusplusdata[i] === undefined))
                                {
