...
export default function Csharp() {
    ...
    let usewebdata = webdata && webdata.filter(m=>m.id ==="csharp-videos");
    ...
    useEffect(() => {
      ...
      settitle(usewebdata&&usewebdata[0].videos[0].title)
        ...
        //quiz code
        let usequizzes = webdata && webdata.filter(m=>m.id ==="csharp-quizzes");
        ...
        const quizclickhandle=async(tag)=>{
            ...
            function msToTime(s) {
              ...
              if(data && data[0].csharpq ==undefined){
                setcurrenttag(tag);
            ...
            else if(data[0].csharpq[tag] ===undefined)
            {
            ...
            else if((data && data[0].csharpq ==undefined) || (data && data[0].csharpq[tag]< new Date().getTime()))
            {
                setcurrenttag(tag);
            setright(usequizzes && usequizzes[0][currenttag][currentq].correcta);
            setquizcheck(quizcheck+1);
            ...
            else
            {
                if(localStorage.getItem("codetime")>new Date().getTime())
                {
                    Swal.fire({
                        icon: 'error',
                        title: 'حاول مجددا بعد ' +msToTime( data && data[0].csharpq[tag]- new Date().getTime())
                    })
                }
                else
                {
        ...
        const TheEndOfQuiz=async()=>{
            if(score/lengthq>=0.5)
            {
                //xp
                let xp= score*100;
                if(data[0].csharpxp ==undefined || data[0].csharpxp[currenttag]<xp || data[0].csharpxp[currenttag] === undefined)
                {
                    let newFields;
                    ...
                    {
                        if(data[0].csharpxp !== undefined){
                            if(data[0].csharpxp[currenttag] !== undefined){
                                newFields={exp:(data[0].exp+xp-data[0].csharpxp[currenttag])}
                            }
                            else{
                    ...
                    <div className="listside">
    ...
    
                        {
                            usewebdata && usewebdata[0].videos.map((item,i) => {
                                ...
                                else if(i===0 &&data[0].csharp==undefined)
                                {
                                    return(
                                        <Videol status="f" title={item.title} keyt={i} key={i} handleclick={clickhandle} quizclick={quizclickhandle}/>
                                        )
                                }
                                else if(data[0].csharp && data[0].csharp[i]===false){
                                    return(
                                    <Videol status="f" title={item.title} keyt={i} key={i} handleclick={clickhandle} quizclick={quizclickhandle}/>
                                    )
                                }else if(data[0].csharp && data[0].csharp[i] === true)
                                {
                                    return <Videol status="t" title={item.title} keyt={i} key={i} handleclick={clickhandle} quizclick={quizclickhandle}/>
                                }else if(data[0].csharp ==undefined || (data[0].csharp && data[0].csharp[i] === undefined))
                                {
