...
export default function Unity() {
    ...
    let usewebdata = webdata && webdata.filter(m=>m.id ==="unity-videos");
    const fram = useRef(null);
    const videoslist = useRef(null);

    if(!buy){
        if(data && data[0].level <5)
        {
    ...
    //quiz code
        let usequizzes = webdata && webdata.filter(m=>m.id ==="unity-quizzes");
        ...
        const quizclickhandle=async(tag)=>{
            ...
            if(data&&data[0].unityq[tag] ===undefined)
            {
            ...
            else if(data && data[0].unityq[tag]< new Date().getTime())
            {
                setcurrenttag(tag);
            setright(usequizzes && usequizzes[0][currenttag][currentq].correcta);
            setquizcheck(quizcheck+1);
            ...
            else
            {
                if(localStorage.getItem("codetime")>new Date().getTime())
                {
                    Swal.fire({
                        icon: 'error',
                        title: 'حاول مجددا بعد ' +msToTime( data && data[0].unityq[tag]- new Date().getTime())
                    })
                }
                else
                {
        ...
        const TheEndOfQuiz=()=>{
            if(score/lengthq>=0.5)
            {
                //xp
                let xp= score*100;
                if(data[0].unityxp[currenttag]<xp || data[0].unityxp[currenttag] === undefined)
                {
                    let newFields;
                    ...
                    {
                        if(data[0].unityxp[currenttag] !== undefined){
                            newFields={exp:(data[0].exp+xp-data[0].unityxp[currenttag])}
                        }
                        else{
                ...
                <div className="Quiz" >
                    <div className="quiz-app"ref={quizblock}>
                        ...
                        <div className="quiz-info">
                            <div className="category">Category: <span>unity</span></div>
                    ...
                    <div className="listside">
    ...
    
                        {
                            usewebdata && usewebdata[0].videos.map((item,i) => {
                                ...
                                else if(data && data[0].unity[i]===false){
                                    return(
                                    <Videol status="f" title={item.title} keyt={i} key={i} handleclick={clickhandle} quizclick={quizclickhandle}/>
                                    )
                                }else if(data && data[0].unity[i] === true)
                                {
                                    return <Videol status="t" title={item.title} keyt={i} key={i} handleclick={clickhandle} quizclick={quizclickhandle}/>
                                }else if(data && data[0].unity[i] === undefined)
                                {
